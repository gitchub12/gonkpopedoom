...
class PamphletProjectile {
  ...
  checkHit() {
    const allTargets = [...r2Units, ...stormtroopers, ...probeDroids, ...xeroxDroids, ...aliens, ...alienBs, ...npcs];
...
// === ALIEN B (GUNGAN) CLASS ===
class AlienB {
  constructor(position, id) {
    this.id = id; 
    this.position = position.clone(); 
    this.health = 2; // Only 2 hit points as requested
    this.mesh = null; 
    this.shadowMesh = null;
  ...
  createMesh() {
    ...
    this.mesh = new THREE.Mesh(alienGeometry, alienBMaterials && alienBMaterials[0] ? alienBMaterials[0] : new THREE.MeshBasicMaterial({color: 0x00ffff}));
    this.mesh.position.copy(this.position); 
    this.mesh.position.y = 1.720; // Same height as AlienA
  ...
  animate() {
    if (this.dying) return; 
    this.animFrame = (this.animFrame + 1) % 8; // 8 animations as specified
    if (this.mesh && alienBMaterials && alienBMaterials.length === 8) { 
      this.mesh.material = alienBMaterials[this.animFrame]; 
    }
  }
  ...
  destroy() {
    ...
    if (this.shadowMesh) scene.remove(this.shadowMesh);
    if (this.glowMesh) scene.remove(this.glowMesh);
    const index = alienBs.indexOf(this); 
    if (index > -1) alienBs.splice(index, 1);
  }
}
class AlienA {
  constructor(position, id) {
...
function createAllUnits() {
  ...
  try {
    ...
    // Room 1 (z: 10-20): Probe droids + NEW: AlienB (Gungan)
    ...
    const alienB1 = new AlienB(new THREE.Vector3(0, 0, 14), 'gungan_room2'); // NEW: Gungan in room 1
    probeDroids.push(probe1, probe2);
    alienBs.push(alienB1);
    ...
    // Room 4 (z: 40-50): Aliens + NEW: AlienB (Gungan)
    ...
    const alienB2 = new AlienB(new THREE.Vector3(0, 0, 44), 'gungan_room5'); // NEW: Gungan in room 4
    aliens.push(alien1, alien2);
    alienBs.push(alienB2);
    ...
    console.log(`Created units: ${r2Units.length} R2s, ${stormtroopers.length} stormies, ${probeDroids.length} probes, ${xeroxDroids.length} xerox, ${aliens.length} aliens, ${alienBs.length} gungans`);
  } catch (error) {
